...
var shareUltimateTrust = shareTrustEnabled ? config.poolServer.shareTrust.ultimate : false;
...
function printStatistic() {
    var statistic = {
        totalMiners: Object.keys(connectedMiners).length,
        loggedMiners: Object.values(connectedMiners).filter(function(m) { return m.authenticated; }).length,
        longerThan20: connectedLongerThan(20).length,
        longerThan60: connectedLongerThan(60).length,
        longerThan300: connectedLongerThan(300).length,
        memoryUsage: process.memoryUsage()
    };
    log('warn', logSystem, 'Statistic %j', [statistic]);
}

setInterval(printStatistic, 10000);

function connectedLongerThan(seconds) {
    return Object.values(connectedMiners).filter(function(miner) {
        var wasConnectedSeconds = ((Date.now() - miner.connectedAt) / 1000 | 0);
        return wasConnectedSeconds >= seconds;
    });
}
...
function getBlockTemplate(callback){
    var params = {reserve_size: 8, wallet_address: config.poolServer.poolAddress};
    apiInterfaces.rpcDaemon('getblocktemplate', params, function(error, result) {
        if (error){
            log('error', logSystem, 'Error polling getblocktemplate %j', [error]);
            callback(false);
            return;
        }
        if (currentBlockTemplate && currentBlockTemplate.height >= result.height) {
            log('warn', logSystem, 'Already got a block template of the same height. Possible race condition');
            return;
        }
...
function getBlockHeight(callback) {
    apiInterfaces.jsonRequestDaemon('/getheight', '', callback);
}
...
function jobRefresh(loop, callback){
    ...
    getBlockHeight(function(error, result) {
        ...
        if (error){
            log('error', logSystem, 'Error polling getheight %j', [error]);
            callback(false);
            return;
        }
...
(function init(){
    jobRefresh(true, function(sucessful){
        ...
        startStratumServer(function(successful){
...
function Miner(id, startingDiff, communicator){
    this.id = id;
    this.login = 'unathenticated';
    this.authenticated = false;
    this.ip = communicator.remoteAddress;
    this.pushMessage = communicator.pushMessage.bind(communicator);
    this.heartbeat();
...
Miner.prototype = {
    ...
    getTargetHex: function(){
        ...
        var buffReversed;
        if(buff.reverse) {
          buffReversed = buff.reverse();
        } else {
          var buffArray = buff.toJSON();
          buffArray.reverse();
          buffReversed = new Buffer(buffArray);
        }
...
function shouldTrust(miner) {
    return miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability;
}
...
function processShare(miner, job, blockTemplate, nonce, resultHash){
    ...
    if (shareTrustEnabled && (shareUltimateTrust || shouldTrust(miner))) {
    ...
    var hashBuff;
    if(hash.reverse) {
      hashBuff = hash.reverse();
    } else {
      var hashArray = hash.toJSON();
      hashArray.reverse();
      hashBuff = new Buffer(hashArray);
    }
    var hashNum = bignum.fromBuffer(hashBuff);
...
function isValidLogin(login) {
    return login.match(/^[a-zA-Z0-9_]{1,100}$/);
}

function handleMinerMethod(miner, method, params, ip, portData, sendReply, communicator){
    ...
    switch(method){
        case 'login':
            ...
            if (!isValidLogin(params.login)){
                sendReply('invalid address used for login');
                return;
            }
            miner.authenticate(params.login);
            sendReply(null, {
                id: miner.id,
                job: miner.getJob(),
                status: 'OK'
...
function initMiner(portData, communicator) {
    var minerId = utils.uid();
    miner = new Miner(minerId, portData.difficulty, communicator);
...
function startStratumServer(callback) {
    stratum.start(function(communicator, portData) {
        var miner = initMiner(portData, communicator);
        communicator.on('message', function(jsonData){
          var sendReply = function(error, result){
            communicator.sendReply(jsonData.id, error, result);
          };
